...
//
// Function : f_kin( Vector3 pos0, Matrix3 R0, double j_param[],
//                   MODEl )
//
//            compute Forward kinematics
//
//            Input  : position & rotation of Link0 - link_pos
//                     joint parameter - j_param
//
...
void f_kin( Vector3 pos0, Matrix3 R0, double *j_param,
	    MODEL &m )
{
    int i, j;
    Matrix3 tmp1,tmp2;
	...
	for( i=1 ; i<m.LINKNUM ; i++ )
	{
	    ...
	    if( m.J_type[i] == 0 ) tmp3[2] = j_param[i];
	...
	for( i=1 ; i<m.LINKNUM ; i++ )
	{
		...
		for ( j=0 ; j<3 ; j++ )
		{
			...
			+ m.link_R[m.JA[i][1]][3*j+2]*j_param[i]/2;
		    
		    m.link_pos[i][j] = m.joint_pos[i][j]
			+ m.link_R[m.JA[i][1]][3*j]  *m.Jto[i][0]
			+ m.link_R[m.JA[i][1]][3*j+1]*m.Jto[i][1]
			+ m.link_R[m.JA[i][1]][3*j+2]*( m.Jto[i][2] + j_param[i]/2 );
		}
	    }
	}
