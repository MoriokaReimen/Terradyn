...
void model_param( string filename, MODEL &m )
{
    ...
    for( i=0 ; i<m.LINKNUM ; i++ ) {
        ...
        ifs >> str >> m.link_I_fixed[i][0*3+1];
        m.link_I_fixed[i][1*3+0] = m.link_I_fixed[i][0*3+1];
        ifs >> str >> m.link_I_fixed[i][0*3+2];
        m.link_I_fixed[i][2*3+0] = m.link_I_fixed[i][0*3+2];
        ifs >> str >> m.link_I_fixed[i][1*3+2];
        m.link_I_fixed[i][2*3+1] = m.link_I_fixed[i][1*3+2];
    }
